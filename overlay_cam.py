...
def get_args():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            'enlarged_cardiomediastinum', 'lung_lesion', 'lung_opacity',
            'normal', 'pleural_effusion', 'pneumonia', 'pneumothorax'
        ],
        help='Target label for GradCAM. If not set, uses the top-predicted label.'
    )
    parser.add_argument(
        '--model', type=str, required=True,
...
if __name__ == '__main__':
    ...
    # load model
    num_classes = 11  # vì bạn có 11 label bệnh
    model = medical_model.get_model(num_classes=num_classes, model_name=args.model)
    state_dict = torch.load(args.checkpoint, map_location="cpu")
    model.load_state_dict(state_dict)
    model.eval()

    # chọn target layer cho GradCAM
    ...
    target_idx = label_map[args.label] if args.label else None
    ...
    warnings.filterwarnings("ignore", category=UserWarning)
    cam = GradCAM(model=model, target_layer=target_layer)
    pred_idx, mask = cam(image, target_idx)
    print(f'✅ GradCAM generated for label "{idx_to_label[pred_idx]}".')

    # overlay
    image = utils.deprocess_image(image)
    image = apply_mask(image, mask)
    utils.save_image(image, args.output_path)
    print(f"💾 Saved CAM to {args.output_path}")